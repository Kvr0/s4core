...
# オーバーフローしてたらtag追加
    execute if score $CloneFlagIndex temporary matches 00.. run data modify storage usedtoblock: result.tag.EntityTag.Tags append value UsedToBlock.0-0
    execute if score $CloneFlagIndex temporary matches ..-1 run data modify storage usedtoblock: result.tag.EntityTag.Tags append value UsedToBlock.0-1
# 0になるまでやる
    scoreboard players operation $CloneFlagIndex temporary *= $2 temporary
